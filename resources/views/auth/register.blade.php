...
<body class="bg-slate-50 w-full min-h-screen flex flex-col justify-between relative overflow-x-hidden">
    ...
    <main class="flex-grow flex flex-col justify-center items-center py-16 z-10 relative">
        <div class="flex flex-col justify-center items-center w-full">
            ...
            <div class="flex items-center gap-2 mb-2">
                <i class="fa-solid fa-graduation-cap text-teal-900 text-2xl"></i>
            ...
            <div class="bg-white w-full max-w-sm rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="flex border-b border-slate-100">
                    <button id="tab-penerima" onclick="switchTab('penerima')"
                        class="flex-1 py-3.5 text-sm font-display role-tab-active transition-all duration-200">
                        Penerima
                    </button>
                    <button id="tab-penyalur" onclick="switchTab('penyalur')"
                        class="flex-1 py-3.5 text-sm font-display role-tab-inactive transition-all duration-200">
                        Penyalur
                    </button>
                </div>

                {{-- Form --}}
                <form action="{{ route('register.store') }}" method="POST" class="px-8 py-7 space-y-5">
                    @csrf
                    <input type="hidden" name="role" id="input-role" value="penerima">

                    {{-- Nama --}}
                    <div>
                        <label for="nama_lengkap" class="block text-xs font-semibold text-slate-700 mb-1.5">Nama
                            Lengkap</label>
                        <div class="relative">
                            <i
                                class="fa-solid fa-user absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm"></i>
                            <input type="text" id="nama_lengkap" name="nama_lengkap"
                                value="{{ old('nama_lengkap') }}" placeholder="John Doe" required
                                class="scholr-input w-full pl-9 pr-4 py-2.5 rounded-xl border border-slate-200 bg-white text-sm text-slate-800 placeholder:text-slate-400 transition-colors duration-200 @error('nama_lengkap') border-red-400 @enderror">
                        </div>
                        @error('nama_lengkap')
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1.5">
                            ...
                            <a href="" class="text-xs text-teal-700 hover:underline font-medium">Lupa
                                password?</a>
                        </div>
                        <div class="relative">
                            <i
                            ...
                            <i id="togglePassword"
                                class="fa-solid fa-eye absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm hover:text-teal-700 transition-colors cursor-pointer"
                                onclick="togglePwd()"></i>
                        </div>
                        @error('password')
                            <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                    ...
                    <p class="text-center text-xs text-slate-500 pt-1">
                        Sudah punya akun?
                        <a href="{{ route('login') }}" class="text-teal-700 font-semibold hover:underline">Masuk
                            sekarang</a>
                    </p>
            ...
            <div class="flex items-center gap-2 mt-6">
                <i class="fa-solid fa-shield-halved text-slate-300 text-base"></i>
                <p class="text-xs text-slate-400 uppercase tracking-widest font-semibold">Dipercaya oleh 50+ Institusi
                    Pendidikan</p>
            </div>
    ...
    <footer class="bg-teal-900 w-full py-12 border-t border-white/10 z-10 relative">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-8 px-10 max-w-7xl mx-auto">
            ...
            <div>
                ...
                <ul class="space-y-3">
                    <li><a class="text-white/70 hover:text-white transition-colors text-sm" href="#">Beranda</a>
                    </li>
                    <li><a class="text-white/70 hover:text-white transition-colors text-sm" href="#">Cara
                            Kerja</a></li>
                    <li><a class="text-white/70 hover:text-white transition-colors text-sm" href="#">Beasiswa</a>
                    </li>
                </ul>
            </div>
            ...
            <div>
                ...
                <ul class="space-y-3">
                    <li><a class="text-white/70 hover:text-white transition-colors text-sm" href="#">Tentang
                            Kami</a></li>
                    <li><a class="text-white/70 hover:text-white transition-colors text-sm" href="#">Kebijakan
                            Privasi</a></li>
                    <li><a class="text-white/70 hover:text-white transition-colors text-sm" href="#">Syarat
                            &amp; Ketentuan</a></li>
                </ul>
            </div>
            ...
            <div>
                ...
                <ul class="space-y-3">
                    <li><a class="text-white/70 hover:text-white transition-colors text-sm"
                            href="mailto:halo@scholr.id">halo@scholr.id</a></li>
                </ul>
                <div class="flex items-center gap-3 mt-5">
                    <a href="#"
                        class="w-8 h-8 rounded-full border border-white/20 flex items-center justify-center hover:border-white/60 transition-colors">
    ...
    <script>
        function switchTab(role) {
            const tabPenerima = document.getElementById('tab-penerima');
            const tabPenyalur = document.getElementById('tab-penyalur');
            const inputRole = document.getElementById('input-role');

            if (role === 'penerima') {
                tabPenerima.classList.add('role-tab-active');
                tabPenerima.classList.remove('role-tab-inactive');
                tabPenyalur.classList.add('role-tab-inactive');
                tabPenyalur.classList.remove('role-tab-active');
            } else {
                tabPenyalur.classList.add('role-tab-active');
                tabPenyalur.classList.remove('role-tab-inactive');
                tabPenerima.classList.add('role-tab-inactive');
                tabPenerima.classList.remove('role-tab-active');
            }
