                    ...
                    <div class="px-6 py-5">
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            @php
                                ...
                                $colorMap = [
                                    'gray'    => ['border' => 'border-gray-500',    'bg' => 'bg-gray-50',    'text' => 'text-gray-700',    'icon' => 'text-gray-500'],
                                    'emerald' => ['border' => 'border-emerald-500', 'bg' => 'bg-emerald-50', 'text' => 'text-emerald-700', 'icon' => 'text-emerald-500'],
                            ...
                            @foreach ($statuses as $value => $meta)
                                ...
                                <label
                                    ...
                                    <div class="text-center">
                                        <p class="status-label text-xs font-semibold {{ $currentStatus === $value ? $c['text'] : 'text-gray-600' }}">
                                            {{ $meta['label'] }}
                                        </p>
                                        <p class="text-[10px] text-gray-400 mt-0.5 leading-tight">{{ $meta['desc'] }}</p>
                                    </div>
                                    @if ($currentStatus === $value)
                                        <div class="status-dot absolute top-2 right-2 w-4 h-4 rounded-full border {{ $c['border'] }} flex items-center justify-center {{ $c['bg'] }}">
                                            <div class="w-2 h-2 rounded-full {{ $c['icon'] === 'text-gray-500' ? 'bg-gray-500' : str_replace('text-', 'bg-', $c['icon']) }}"></div>
                                        </div>
                                    @else
                                        <div class="status-dot hidden absolute top-2 right-2 w-4 h-4 rounded-full border flex items-center justify-center">
